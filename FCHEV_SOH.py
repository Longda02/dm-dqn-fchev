...
class FCHEV_SOH:
    def __init__(self):
        ...
        self.P_FC_high = 0.9 * self.P_FC_max        # 已有论文中的说法是82开  燃料电池高功率阈值
        self.P_FC_low = 0.1 * self.P_FC_max
        self.P_FC_off = 0.01 * self.P_FC_max        # 燃料电池关闭功率
    ...
    # fuel cell model
    def _func_init(self):
        data_dir = "F:/Zhang Longda/Reinforcement Learning/chen/Test/data1/"  # car data from BIT
        # fuel cell engine
        ...
        convert_coefficient_data = scio.loadmat('F:/Zhang Longda/Reinforcement Learning/chen/Test/data1/conversion.mat')
        convert_coefficient = convert_coefficient_data['conversion'][0]
        p_fcs_data = scio.loadmat('F:/Zhang Longda/Reinforcement Learning/chen/Test/data1/P_fc_conv.mat')
        ...
        self.mot_trq_min = interp1d(mot_trq_min[:, 0], mot_trq_min[:, 1], kind='linear', fill_value='extrapolate') # 转速与转矩
    ...
    def _FCE_is_ON(self, P_FCE):    # 燃料电池状态判断
    ...
    def run_FC_SOH(self, P_fc):
        ...
        # start-stop cycles
        if self._FCE_is_ON(self.P_FC_old) is False and self._FCE_is_ON(P_fc) is True:# 关闭到打开导致的电池退化
            # d_s_s = self.simtime * 1.96 * 1e-3 / 3600
            d_s_s = self.simtime*1.96
        else:
            d_s_s = 0
        # low-power load condition
        if self._FCE_is_ON(P_fc) is True and P_fc < self.P_FC_low:  # 低功率负载退化
        ...
        if P_fc >= self.P_FC_high:
            ...
            d_high = self.simtime*1.47  # 高功率负载退化
        else:
            d_high = 0
        # load changing cycles
        # d_l_c = 5.93*1e-5*abs(P_fc-self.P_FC_old)/(self.P_FC_high-self.P_FC_low)
        d_l_c = 5.93*abs(P_fc-self.P_FC_old)/(self.P_FC_high-self.P_FC_low) # 负载变化退化
...
if __name__=='__main__':
    ...
    plt.plot(pfcs, yconv)
    plt.show()
    # P_batt = 50000  # 例如，50kW
    # SOC = 0.5  # 初始SOC为50%
    #
    # SOC_delta, SOC_new, done, out_info = fchev.run_power_battery(P_batt, SOC)
    # print(f"SOC变化量: {SOC_delta}")
    # print(f"新的SOC: {SOC_new}")
    # print(f"是否失败: {done}")
    # print(f"输出信息: {out_info}")
    #
    # P_fc = 60
    # FCS_De,out_info = fchev.run_FC_SOH(P_fc)
    # print(f"退化了: {FCS_De}")
    
# # fuel cell engine
    # data = scio.loadmat(data_dir + 'P_fc.mat')
    # P_fc = data['fc_pwr_map'][0]  # attention: kW
    # data = scio.loadmat(data_dir + 'P_fce.mat')
    # P_fce = data['fc_pwr_map'][0]  # attention: kW
    # data = scio.loadmat(data_dir + 'h2_consumption.mat')
    # h2_consumption = data['fc_fuel_map'][0]
    # data = scio.loadmat(data_dir + 'fce_eff.mat')
    # fce_eff = data['fc_eff_map'][0]
    # self.P_fce = interp1d(P_fc, P_fce, kind='linear', fill_value='extrapolate')
    # self.fce_eff = interp1d(P_fc, fce_eff, kind='linear', fill_value='extrapolate')
    # self.h2_consumption = interp1d(P_fc, h2_consumption, kind='linear', fill_value='extrapolate')
    # # 电池功率到等效氢气消耗量的转换系数，动态变化
    # convert_coefficient_data = scio.loadmat('F:/Zhang Longda/Reinforcement Learning/chen/Test/data/conversion.mat')
    # convert_coefficient = convert_coefficient_data['conversion'][0]
    # p_fcs_data = scio.loadmat('F:/Zhang Longda/Reinforcement Learning/chen/Test/data/P_fc_conv.mat')
    # p_fcs = p_fcs_data['P_fc_conv'][0]        # kW 60-point
    # self.h2_conv_coef = interp1d(p_fcs, convert_coefficient, kind='linear', fill_value='extrapolate')# 功率与系数一维插值
    #
    # # motor
    # data = scio.loadmat(data_dir + 'mot_eff.mat')
    # motor_eff = data['mc_eff_map']
    # data = scio.loadmat(data_dir + 'W_mot.mat')
    # motor_spd = data['mc_map_spd'][0]
    # data = scio.loadmat(data_dir + 'T_mot.mat')
    # motor_trq = data['mc_map_trq'][0]
    #
    # # 这三行新加的
    # motor_spd = np.array(motor_spd).ravel()  # 将行向量转为 1D 数组
    # motor_trq = np.array(motor_trq).ravel()  # 同样转换 motor_trq 为 1D 数组
    # motor_eff = motor_eff.T  # 转置，形状变为 (21, 11)
    #
    # self.mot_eff_map = interp2d(motor_spd, motor_trq, motor_eff)
    #
    # data = scio.loadmat(data_dir + 'mot_trq_min.mat')
    # mot_trq_min = data['mc_max_gen_trq']
    # self.mot_trq_min = interp1d(mot_trq_min[:, 0], mot_trq_min[:, 1], kind='linear', fill_value='extrapolate')
    #
    # data = scio.loadmat(data_dir + 'mot_trq_max.mat')
    # mot_trq_max = data['mc_max_trq']
    # self.mot_trq_max = interp1d(mot_trq_max[:, 0], mot_trq_max[:, 1], kind='linear', fill_value='extrapolate')
    #
    # # DC/DC
    # data = scio.loadmat(data_dir + 'e_dcdc.mat')
    # DCDC_eff = data['dcdc_eff'][0]
    # data = scio.loadmat(data_dir + 'P_fce.mat')
    # P_DCDC = data['fc_pwr_map'][0]  # kW
    # self.DCDC_eff = interp1d(P_DCDC, DCDC_eff, kind='linear', fill_value='extrapolate')