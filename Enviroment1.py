...
class env(object):
    ...
    def feedback(self, a_index, s_index):
        ...
        future_power_expectation = np.dot(future_power_set, probability_cur2fut)  # 需求功率的离散处理（1x47 47x1）
        n = int(future_power_expectation)
        m = future_power_expectation - n
        if m >= 0.25 and m < 0.75:
            m = 0.5
        ...
        Pfc_input = a
        # Pbat_input = 1 - a
        # Pfc_input = action_space.values[0, a_index]
        ...
        # print("Reward=====",Reward)
        # r = -(Reward - 0.00000000001) / (1500 - 0.00000000001)##################
        r = -(Reward - 0.00228) / (1233.714 - 0.00228)
        # r = -(Reward - 0.128) / (150 - 0.1228)
        # r = -(Reward - 0.000001) / (1500 - 0.000001)
        # if (a >= 0.95) or (a <= 0.05):
        #       r = r - 1
        if (np.isnan(r)):
              r = -1
